...
const Auth = () => {
  ...
  React.useEffect(() => {
    axios.post("/api/login").then((v) => {
      const urlNext = queryParams.get("next");
      const data = v.data as UserReducerType;

      dispatch({ type: "USER_SET", payload: data });
      return <Navigate to={urlNext || "/"} />;
    });
  ...
  return (
    <section className="flex flex-1 justify-center items-center">
      <div className="relative w-full h-full sm:h-auto sm:max-w-lg">
        ...
        <div className="w-full h-full px-4 sm:rounded-lg bg-white bg-opacity-5 shadow-md sm:border sm:border-solid sm:border-slate-700 sm:dark:border-slate-200 sm:border-opacity-50 sm:dark:border-opacity-50 backdrop-blur-sm flex flex-col justify-center items-center sm:p-12">
